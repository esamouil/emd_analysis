...
# the adc_threshold is used since the marker rows give huge adc values, so I use that to separate the measurements
...
#give which timewindow to show by the measurement_index:

measurement_index=1

# get a measurement 
example_meas = df[df["meas_id"] == measurement_index]

# plot
fig, ax = plt.subplots(figsize=(10,5))
ax.plot(example_meas["timestamp"], example_meas["adc_value"]*config['adc_to_voltage'])
...
# Two peaks per measuremt since for each chopper rotation we have two bunches
