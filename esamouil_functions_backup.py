...
def split_measurements_inplace(df, adc_threshold, id_col="meas_id"):
    """
...
def shift_measurements_timestamps(df):
    ...
    Args:
        df (pd.DataFrame): must have 'timestamp' and 'meas_id'

    Returns:
        pd.DataFrame: df with shifted timestamps (in place)
    """
    for mid, dfi in df.groupby("meas_id"):
        t0 = dfi["timestamp"].iloc[0]
        df.loc[df["meas_id"] == mid, "timestamp"] -= t0

    return df

def shift_measurements_timestamps_2(df):
    """
    Shift timestamps in each measurement so the first timestamp is 0.
    """
