...
with open("config_large_run.json", "r") as f:
    config = json.load(f)
...
txt_file = Path("/home/esamouil/analysis/data_stor/local_commissioning_data/004_dream_cc_2026/Z220126_IBM1_0008.txt")          # your config TXT
parquet_file = base_folder / config["parquet_file"]

folder = parquet_file.parent
save_outputs = True   # or from config

# -----------------------------
# Setup output directories
# -----------------------------
output_dir = folder / parquet_file.stem
if save_outputs:
    import shutil
...
adc_dir = output_dir / "adc"
voltage_dir = output_dir / "voltage"
adc_sparks_dir = adc_dir / "sparks"
voltage_sparks_dir = voltage_dir / "sparks"
...
if save_outputs:
    log_file_path = output_dir / "analysis.log"
...
print("Parquet File:", parquet_file.name)
print("===================================\n")    
...
# -----------------------------
print("=============== Data Import ===============")
# Decide binary/ASCII based on TXT flag
if info["event_binary"]:   # usually only relevant if using EMD
    # Since we already have Parquet, you can just read it:
    import pandas as pd
    df = pd.read_parquet(parquet_file)
else:
    # For ASCII, if you ever still use a small EMD, fill gaps
    df = read_ascii_emd(parquet_file)  # or keep your ASCII path
...
#%% Fix wraparound in timestamps
step = info["tia_summation_points"] * info["tia_sampling_ns"] * 1e-3  # define your step value in the config

timestamps = df["timestamp"].to_numpy()  # view of the column, not a copy

for i in range(1, len(timestamps)):
    if timestamps[i] <= timestamps[i-1]:
        timestamps[i] = timestamps[i-1] + step

# Assign back in-place (optional since .to_numpy() gives a view for numeric types)
...
#%% Histogram of the values (with Gaussian fit)
...
# Show and optionally save figure
fig.show() if not save_outputs else None
...
# Save histogram as a dataframe (bin centers + counts)
hist_df = pd.DataFrame({
    "bin_center": bin_centers,
    "counts": counts
})

hist_save_path = voltage_dir / "histogram_dataframe.parquet"
hist_df.to_parquet(hist_save_path, index=False)
print(f"Saved histogram dataframe to: {hist_save_path}")

# Free memory
plt.close(fig)
del values_to_fit, counts, bins, bin_centers, popt, A, mu, sigma, errors, chi2, ndf, chi2_ndf, textstr, hist_df
...
peaks = detect_fft_peaks(fft_df.loc[fft_df["frequency"].between(0, 1000)], neighborhood=1000, top_n=10)
...
fig.show() if not save_outputs else None
save_plot(fig, "User_Range_with_peaks", fft_dir, save_outputs)
...
fft_save_path = fft_dir / "fft_dataframe.parquet"
